...
Communicate in French. Code and comments in English.
...
- When any workflow, setup step, or process changes, update GUIDE.md to match — it must always reflect the current state.
...
10. PRODUCT.md — register, users, brand personality, anti-references (generated by `/impeccable teach` — not in repo until first run)
11. DESIGN.md — colors, typography, components, layout (generated by `/impeccable document` — not in repo until first run)
...
- Run `/audit` for design system compliance scan (pnpm audit:project)
...
5. Run `pnpm design` to generate baseline tokens from the brief
6. Record design decisions in `design-system/decisions.md`
7. Run `pnpm dev` and start building with `/impeccable craft`
...
4. Claude applies `client.config.ts` → `pnpm setup` → `/impeccable teach` → `pnpm design`
...
- BOILERPLATE-ISSUES.md tracks friction encountered on client projects
- After each project, document issues and fixes to improve the boilerplate for next time
- The `/audit` skill (category 9) automatically extracts improvement proposals from session history
